...
class Layer: 
    ...
    def backward_step_wooks(self, target):
        ...
        for b, w in zip(self.bias_vector, self.weight_matrix):
            preact = self.layer_preactivation # reusing the forward_step() method from the parent class Layer
            list_of_preact.append(preact)

            activation = self.forward_step(input)  # reusing the forward_step() method from the parent class Layer
            list_of_act.append(activation)
...
class MLP(Layer):
    ...
    def forward_step_wookis_ver(self, input):

        for b, w in zip(self.bias_vector, self.weight_matrix):
            input = self.forward_step(input)  # reusing the forward_step() method from the parent class Layer

        # eventually "input" of the input layer becomes overwritten as "output" of the output layer
        return input

    def backward_step_wookis_ver(self, x, target):
